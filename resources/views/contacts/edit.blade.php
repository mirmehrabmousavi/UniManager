    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">ویرایش تماس</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">خانه</a></li>
                            <li class="breadcrumb-item active">ویرایش تماس</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-12 col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">ویرایش تماس</h4>
                        </div>
                        <div class="card-content">
                            <div class="card-body">
                                <form class="form form-vertical" action="{{ route('contact.update',[$contact->id]) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <div class="form-body">
                                        <div class="row">
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="date" id="contact-platform" class="form-control @error('date') is-invalid @enderror"
                                                           name="date" value="{{$contact->date}}" required>
                                                </div>
                                                @error('date')
                                            ...
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control @error('contact_platform') is-invalid @enderror"
                                                        ...
                                                        @if($contact_platform)
                                                            @if($contact->contact_platform == 'null')
                                                                <option value="null">بدون دلیل</option>
                                                            @endif
                                                            @foreach($contact_platform as $item)
                                                                <option value="{{$item->name}}" @if($item->name == $contact->contact_platform) selected @endif>{{$item->name}}</option>
                                                            @endforeach
                                                        @else
                                                            @if($contact->contact_platform == 'null')
                                            ...
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control @error('contact_reason') is-invalid @enderror"
                                                        ...
                                                        @if($contact_reason)
                                                            @if($contact->contact_reason == 'null')
                                                                <option value="null">بدون دلیل</option>
                                                            @endif
                                                            @foreach($contact_reason as $item)
                                                                <option value="{{$item->name}}" @if($item->name == $contact->contact_reason) selected @endif>{{$item->name}}</option>
                                                            @endforeach
                                                        @else
                                                            @if($contact->contact_reason == 'null')
                                                                <option value="null">بدون دلیل</option>
                                            ...
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control @error('contact_description') is-invalid @enderror"
                                                              name="contact_description" required>{{$contact->contact_description}}</textarea>
                                                </div>
